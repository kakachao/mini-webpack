...
const baseDir = normalizePath(process.cwd()); //获取工作目录，在哪里执行命令就获取哪里的目录，这里获取的也是跟操作系统有关系，要替换成/
//获取文件路径
...
class Compiler {
  ...
  run(callback) {
    ...
    const onCompiled = (err, stats, fileDependencies) => {
      ...
      let distDir = this.options.output.path;
...
class Compilation {
  ...
  build(callback) {
    ...
    if (typeof this.options.entry === 'string') {
      //如果是单入口，将entry:"xx"变成{main:"xx"}，这里需要做兼容
